...
if not os.path.exists("plots"):
    os.mkdir("plots")
if not os.path.exists("plots/architectures3"):
    os.mkdir("plots/architectures3")
if not os.path.exists("tables"):
    os.mkdir("tables")
if not os.path.exists("tables/architectures3"):
    os.mkdir("tables/architectures3")
...
for dataset in datasets:
    ...
    for layer_size in layer_sizes:
        nn = NeuralNet(dataset.x_train.shape[1], weight_init=uniform_init, name=f"layers: {first_layer_size}, {first_layer_size}, {layer_size}",
    ...
    save_errors_to_latex(layers_error, [f"layers: {first_layer_size}, {layer_size}" for layer_size in layer_sizes],
                         ...
                         f"tab:architecture3:{dataset.name}_{dataset.size}",
                         "tables/architectures3/{0}_{1}.txt".format(dataset.name, dataset.size),
                         shrink=True)
    ...
    boxplot_labels = [f"{first_layer_size}, {first_layer_size}, {layer_size}" for layer_size in layer_sizes]
    plt.subplot(error_subplots, 2, 1)
    plotter.boxplot_of_errors(layers_error, 2, f'{error_name} train', show=False, labels=boxplot_labels)
    plt.subplot(error_subplots, 2, 2)
    plotter.boxplot_of_errors(layers_error, 3, f'{error_name} test', show=False, labels=boxplot_labels)
    if dataset.task_type == "classification":
        plt.subplot(2, 2, 3)
        plotter.boxplot_of_errors(layers_error, 0, 'Loss train', show=False, labels=boxplot_labels)
        plt.subplot(2, 2, 4)
        plotter.boxplot_of_errors(layers_error, 1, 'Loss test', show=False, labels=boxplot_labels)
    plt.savefig("plots/architectures3/{0}_{1}_loss_boxplot.png".format(dataset.name, dataset.size),
    ...
    plt.savefig("plots/architectures3/{0}_{1}_loss_history.png".format(dataset.name, dataset.size),
                dpi=100, bbox_inches="tight")
    ...
    if dataset.task_type == "classification":
        ...
        for i, size in enumerate(layer_sizes):
            plt.subplot(1, 3, i+1)
            plotter.plot_data_2d(i, title=f"layers: {first_layer_size}, {first_layer_size}, {size}", show=False)
            plt.savefig("plots/architectures3/{0}_{1}_points.png".format(dataset.name, dataset.size),
                        dpi=100, bbox_inches="tight")
    else:
        plt.subplots(1, 1)
        plotter.plot_data_1d(show=False)
        plt.savefig("plots/architectures3/{0}_{1}_values.png".format(dataset.name, dataset.size),
                    dpi=100, bbox_inches="tight")
