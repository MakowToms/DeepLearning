...
if not os.path.exists("plots"):
    os.mkdir("plots")
if not os.path.exists("plots/architectures2"):
    os.mkdir("plots/architectures2")
if not os.path.exists("tables"):
    os.mkdir("tables")
if not os.path.exists("tables/architectures2"):
    os.mkdir("tables/architectures2")
...
for dataset in datasets:
    ...
    save_errors_to_latex(layers_error, [f"layers: {first_layer_size}, {layer_size}" for layer_size in layer_sizes],
                         ...
                         f"tab:architecture2:{dataset.name}_{dataset.size}",
                         "tables/architectures2/{0}_{1}.txt".format(dataset.name, dataset.size),
                         shrink=True)
    ...
    boxplot_labels = [f"{first_layer_size}, {layer_size}" for layer_size in layer_sizes]
    ...
    plt.savefig("plots/architectures2/{0}_{1}_loss_boxplot.png".format(dataset.name, dataset.size),
    ...
    plt.savefig("plots/architectures2/{0}_{1}_loss_history.png".format(dataset.name, dataset.size),
                dpi=100, bbox_inches="tight")
    ...
    if dataset.task_type == "classification":
        ...
        for i, size in enumerate(layer_sizes):
            plt.subplot(1, 3, i+1)
            plotter.plot_data_2d(i, title=f"layers: {first_layer_size}, {size}", show=False)
            plt.savefig("plots/architectures2/{0}_{1}_points.png".format(dataset.name, dataset.size),
                        dpi=100, bbox_inches="tight")
    else:
        plt.subplots(1, 1)
        plotter.plot_data_1d(show=False)
        plt.savefig("plots/architectures2/{0}_{1}_values.png".format(dataset.name, dataset.size),
                    dpi=100, bbox_inches="tight")
